...
import { apiKey, randomTatooImg } from "../../utils/MockData"; // Dados usados para teste.
import {
  CepValidator,
  CnpjValidator,
  EmailValidator,
...
function ArtistProfile() {
  ...
  const [tatuadorImgsrc, setTatuadorImgSrc] = useState("");
  const [tatuagemImgSrc, setTatuagemImgSrc] = useState("");
  ...
  async function GetArtistInfos() {
    const { data } = await api.get(
      `/tatuadores/${localStorage.getItem("id_tatuador_aux")}`
    );
  ...
  async function GetTattoos() {
    api
      .get(`/tatuagens/tatuador/${localStorage.getItem("id_tatuador_aux")}`)
      .then((response) => {
  ...
  async function GetTattooArtistStyles() {
    ...
    const { data } = await api.get(
      `/estilo-tatuador/id-tatuador/${localStorage.getItem("id_tatuador_aux")}`
    );
  ...
  function uncheckStylesCheckbox(params) {
    let selecteds = document
      .getElementById("tattoo-styles")
  ...
  async function GetTattooArtistRatings() {
    try {
      const { data } = await api.get(
        `/avaliacao/media/${localStorage.getItem("id_tatuador_aux")}`
      );
  ...
  async function HandleEdit() {
    ...
    formData.append("image", fileTatuador);
    try {
      await fetch("https://api.imgur.com/3/image/ ", {
        method: "post",
        headers: {
          Authorization: `Client-id 241dc1dec32c2d8`,
        },
        body: formData,
      })
        .then((data) => data.json())
        .then((img) => setTatuadorImgSrc(img.data.link));
      ...
      const userData = {
        ...
        email: email,
        senha: password,
        conta_instagram: instagramIntegration ? instagramUsername : "",
        foto_perfil: tatuadorImgsrc,
        sobre: aboutArtist,
      };
      console.log(userData);

      const { data } = await api.put(
        `/tatuadores/${dataUser.id_tatuador}`,
        userData
      );
      ...
      if (
        dataUser.conta_instagram !== instagramUsername &&
  ...
  async function HandleAddTattoo() {
    setLoading(true);
    const formData = new FormData();
    ...
    await fetch("https://api.imgur.com/3/image/ ", {
      ...
      .then((data) => data.json())
      .then((img) => setTatuagemImgSrc(img.data.link));
    const data = {
      titulo: tattooTitle,
      local_tatuagem: tattooLocal,
      descricao: null,
      src_imagem: tatuagemImgSrc,
      id_tatuador: id,
    };
    await api.post(`/tatuagens`, data).then(setLoading(false));
  }
  ...
  showEditProfile || showAddTattoo
    ? (document.documentElement.style.overflow = "hidden")
    : (document.documentElement.style.overflow = "auto"); // Travar scrool da tela quando o modal de edição estiver aberto.
  ...
  const MapPin = () => (
    <div>
      {" "}
      <img src={MapPinIcon} alt="" className="pin-img" />{" "}
    </div>
  );
    ...
    return (
      <div className="map">
        <GoogleMapReact
          ...
          defaultZoom={8}
        >
          <MapPin lat={pinCenter.lat} lng={pinCenter.lng} />
        </GoogleMapReact>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ left: showEditProfile ? 0 : -700 }}
      >
        <div
          className="edit-profile-container"
          style={{ marginTop: loading ? 140 : 70 }}
        >
                ...
                <FormControlLabel
                  ...
                  label={
                    <div className="instagramOptions">
                      <p style={{ fontSize: 14 }}>
                        {instagramIntegration ? "Ativada" : "Desativada"}
                      </p>
                    </div>
                  }
                />
              ...
              <div className="tattoo-styles-row">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    let value_included = tattooStylesSelected.includes(
                      element.id_estilo
                    );
                    let is_checked = e.target.checked;

                    if (value_included && !is_checked) {
                      let position = tattooStylesSelected.indexOf(
                        element.id_estilo
                      );
                ...
                />
                <p key={element.id_estilo}>{element.titulo}</p>
              </div>
            ))}
          </div>
          <button
      ...
      <div className="add-tattoo" style={{ left: showAddTattoo ? 0 : -700 }}>
        <div
          className="add-tattoo-container"
          style={{ marginTop: loading ? 140 : 70 }}
        >
      ...
      <div className="profile-artist-container">
          ...
          <div className="name-and-photo">
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(Ratings(artistRating)),
              }}
            ></div>
          </div>
        ...
        {!updateStatus && isAdmin && instagramUsername && (
          <div
            className="update-instagram-button"
            id="update-instagram-images"
            onClick={() => {
              showConfirmDialog();
            }}
          >
